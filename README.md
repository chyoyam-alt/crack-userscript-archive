...
"originalSources": [
  {
    "url": "https://첫번째-원본.example",
    "label": "첫 번째 원본",
    "author": "원본 작성자",
    "note": "무엇을 기반으로 했는지"
  },
  {
    "url": "https://두번째-원본.example",
    "label": "두 번째 원본",
    "author": "다른 작성자",
    "note": "함께 참고한 원본"
  }
...
기존 단일 `originalSource` 형식도 계속 지원합니다. 원본이 여러 개면 카드와 상세 화면의 `원본 링크 N개` 버튼에서 각각 선택해 열 수 있습니다.

### 최상단 추천 확프

추천 확프는 일반 목록보다 위의 `추천` 영역에 따로 표시됩니다. 은은한 반짝임 효과는 브라우저의 모션 감소 설정을 따릅니다.

`catalog.source.json`의 확프 항목에 아래 값을 추가하면 추천 영역에 노출할 수 있습니다.
...
`priority`가 큰 항목부터 앞에 표시됩니다. 간단하게 태그 배열에 `"추천"`을 넣어도 추천 항목으로 인식합니다. 실제 공개 화면이 읽는 `data/catalog.json`과 비상용 `data/catalog.fallback.js`에도 같은 추천 정보가 반영되어야 합니다.

### 기기별 설치 방법

상단 `설치안내`에서 PC·iOS·Android 설치 안내문을 각각 새 탭으로 열 수 있습니다.
